...
void onWebSocketEvent(WStype_t type, uint8_t *payload, size_t length) {
    switch (type) {
        ...
        case WStype_CONNECTED: {
            ...
            // Subscribe to the 24hr ticker stream for the specified crypto
            char subscribeMessage[100];
            char cryptoLower[100];
            int crypto_len = strlen(crypto);
            for (int i = 0; i < crypto_len; i++) {
                cryptoLower[i] = tolower(crypto[i]);
            }
            cryptoLower[crypto_len] = '\0';
            snprintf(subscribeMessage, sizeof(subscribeMessage), "{\"method\": \"SUBSCRIBE\", \"params\": [\"%susdt@ticker\"], \"id\": 1}", cryptoLower);
